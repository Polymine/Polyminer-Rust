...
// Vector type for processing multiple nonces
typedef struct {
    ulong n0;
    ulong n1;
    ulong n2;
...
__kernel void hashMessage(
    ...
    // Process four nonces per thread using ulong4
    ulong4 nonces = (ulong4)(nonce_vec.n0, nonce_vec.n1, nonce_vec.n2, nonce_vec.n3);

    for (uint i = 0; i < 4; i++) { // Process each nonce in the vector
        ulong current_nonce;
        switch(i) {
            case 0: current_nonce = nonces.x; break;
            case 1: current_nonce = nonces.y; break;
            case 2: current_nonce = nonces.z; break;
            case 3: current_nonce = nonces.w; break;
            default: current_nonce = 0; break;
        }
        ...
        for (uint j = 0; j < UINT256_LENGTH; j++) {
            if (j < 24u) {
                ...
                message[NONCE_POSITION + j] = (uchar)((current_nonce >> (8 * (UINT64_LENGTH - 1 - (j - 24)))) & 0xFF);
            }
        }
